...
S-SUPERADMIN-PHASE1 (2026-06-25, build-only) — Super-Admin Phase 1 audit-actor foundation (F1+D-D). schema.prisma CoreAuditLog: user_id String→String? + relation onDelete Restrict→SetNull; +actor_name/actor_email/actor_role (String?, nullable). audit.service.ts auditLog(): LOUD pre-create validation (throws + distinct console.error on non-UUID actor — no more silent swallow), persists actor_* snapshot; exportAuditLogsAsCsv null-guards l.user→snapshot fallback. roleRegistry.service.ts: `SYSTEM_ROLE='system'` + isKnownRole() recognises it while it stays OUT of the registry Map (excluded from every list — perms-matrix/configurable/notifiable). index.ts: export SYSTEM_ROLE + re-export SYSTEM_ACTOR_USER_ID from contracts. Migration 20260625000001_superadmin_phase1_audit_actor_snapshot — HAND-AUTHORED (shadow-DB blocked P3006 on `task_status` cross-package) then applied to gracie_uat_mirror via psql + `prisma migrate resolve --applied`: 3 cols, user_id DROP NOT NULL, FK→ON DELETE SET NULL (legacy name audit_logs_user_id_fkey dropped, re-added as core_audit_log_user_id_fkey), idempotent INSERT System core_users row (id=SYSTEM_ACTOR_USER_ID 99d597d8…, role='system', is_active=false, non-login, no tenant/org). prisma generate + tsc -b 0 + npm run build 0. Verified on mirror: cols/nullable/FK/seed + idempotent re-run; rolled-back proof that a user owning an audit row now hard-deletes (user_id→NULL via SetNull, snapshot preserved). Lifts uat.56 hard-delete/tenant-purge Restrict blocker. Branch feature/superadmin-phase1-audit-actor, no version bump, no merge, no UAT deploy. (Contracts: +SYSTEM_ACTOR_USER_ID const + AuditLogParams actorName/email/role.)

S-G1-ASSIGNABLE-ROLES (2026-06-30, build-only) — roleRegistry.service.ts: new getAssignableRoleKeys() = getAllRoleKeys().filter(r => r !== 'super_admin') (assignable via admin user-write endpoints; excludes platform-only super_admin, inherently excludes non-registry system actor, keeps org_admin) + isAssignableRole(roleKey) convenience; both exported from index.ts barrel alongside isKnownRole/getConfigurableRoleKeys. Consumed by Privacy's three user-write role gates (G1 privilege-escalation fix). npm run build + tsc -b 0 errors (exports confirmed in dist .js/.d.ts). NO migration, NO schema change. Branch fix/g1-assignable-roles-gate; not versioned at build time. **MERGED to master (bc0ef85) + DEPLOYED to UAT in the combined G1+G2+G5 batch (Privacy uat.63, 2026-06-30) — dist+package.json SCP-delivered, helper confirmed live in server Core dist.**
...
3. Core build depends on the LOCAL `file:../gracie-contracts` (0.1.8); the registry only publishes the stale `@wings2i-gracie/contracts@0.1.0`, so a clean `npm install` (rm -rf node_modules) pulls 0.1.0 and breaks the build with ~15 TS2305 "no exported member" errors (CoreLicenseTier/StorageProvider/ReportingTemplateSpec/ModuleRegistration/FeedReviewStatus etc.). After ANY clean install in gracie-core, re-run `npm install ../gracie-contracts` to re-link local 0.1.8 BEFORE building. Note: that re-link rewrites package.json's contracts spec to `file:../gracie-contracts` — revert to committed `^0.1.0` before SCP'ing package.json (the server keeps `^0.1.0`; do not ship the file: rewrite). (Established uat.63 deploy.)
